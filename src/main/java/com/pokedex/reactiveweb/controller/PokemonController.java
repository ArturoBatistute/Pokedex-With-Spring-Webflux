...
public class PokemonController {
    ...
    @DeleteMapping("{id}")
    public Mono<ResponseEntity<String>> deletePokemon (@PathVariable(value = "id") String id) {
        return repository.findById(id)
                .flatMap(existingPokemon ->
                    repository.delete(existingPokemon)
                            .then(Mono.just(ResponseEntity.ok().body("Pokemon Deleted!")))
                )
                .defaultIfEmpty(ResponseEntity.notFound().build());
    }

    @DeleteMapping
    public Mono<ResponseEntity<String>> deleteAllPokemons() {
        return repository.deleteAll()
                .then(Mono.just(ResponseEntity.ok().body("All Pokemons are gone!")));
    }
